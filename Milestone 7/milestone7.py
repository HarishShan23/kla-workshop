...
milestone7_source = 'Source.txt'
milestone7_poi = 'POI.txt'
milestone7_output = 'MileStone7_Result.txt'
...
def compare_list(list1,list2):
    for i in range(len(list1)):
        ...
        list_temp = list1[i:] + list1[:i]
        res = [i / j for i, j in zip(list_temp, list2)]
...
def matchPolygons(poly_objList, poi_objList):
    ...
    for i in range(len(poi_objList)):
        for j in range(len(poly_objList)):
            ...
            if poi_objList[i].numVertices == poly_objList[j].numVertices and poi_objList[i].layer==poly_objList[j].layer:
                ...
                if matchVertices(poly_vertexList, poi_vertexList) :
                    ...
                    layersList.append(poly_objList[j].layer)
    
    print(layersList)
    return list(set(matched_indx))

with open(milestone7_poi, 'r') as rf_poi:
...
with open(milestone7_source, 'r') as rf:
    data = rf.read()
...
with open(milestone7_output, 'w') as rw:
    rw.write(header)
